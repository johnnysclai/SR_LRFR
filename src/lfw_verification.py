...
def run(args, fnet, srnet=None, step=None):
    ...
    features11_total = torch.Tensor(np.zeros((6000, args.feature_dim), dtype=np.float32)).to(args.device)
    features12_total = torch.Tensor(np.zeros((6000, args.feature_dim), dtype=np.float32)).to(args.device)
    features21_total = torch.Tensor(np.zeros((6000, args.feature_dim), dtype=np.float32)).to(args.device)
    features22_total = torch.Tensor(np.zeros((6000, args.feature_dim), dtype=np.float32)).to(args.device)
    labels = torch.Tensor(np.zeros((6000, 1), dtype=np.float32)).to(args.device)
    with torch.no_grad():
        bs_total = 0
